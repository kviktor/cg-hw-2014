...
void glSphere(bool in=false) {
    ...
    for(int v=0; v<SPHERE_NTESS; v++) {
        for(int u=0; u<SPHERE_NTESS + (in ? 1 : 0); u++) {
...
bool rockets[6];
Vector sat_speed(0.00001, 0.0001, 0.0001);
Vector sat_position(-5,2,1);
long old_time;
...
void glFire() {
    glMaterialfv( GL_FRONT, GL_DIFFUSE, fire);
    glRotatef(180, 1, 0, 0);
    glPushMatrix();
        glCube();
        glPushMatrix();
...
void onInitialization( ) {
    ...
    for(int i=0; i<6; rockets[i++] = false);

    old_time = glutGet(GLUT_ELAPSED_TIME);
}

float rot_spaceship = 0;
float rot_planet = 0;
float rot_sat = 0;

float orbit_spaceship = 0;

bool camera_crew = false;
...
void onDisplay( ) {
    ...
    float angl = orbit_spaceship*M_PI/180.0f;
    float radius = 6.5f;
    Vector rr(radius*cos(angl),0,-1.0*radius*sin(angl));
    Vector rc((radius-1+0)*cos(angl+3.14/6.0),0,-1.0*(radius-1+0)*sin(angl+3.14/6.0));

    rr = Vector(rr.x *cos(-3.14/4.0) - rr.y *sin(-3.14/4.0), rr.x * sin(-3.14/4) + rr.y - cos(-3.14/4), rr.z);
    rc = Vector(rc.x *cos(-3.14/4.0) - rc.y *sin(-3.14/4.0), rc.x * sin(-3.14/4) + rc.y - cos(-3.14/4), rc.z);
    rr.y += 1.0f;
    rc.y += 1.0f;

    Vector cdir = (rc-rr).normalize();
    Vector camp = rr;

    Vector up = (cdir % (rr*-1)).normalize();;
    glPushMatrix();
        glLoadIdentity();
        if(camera_crew)
            gluLookAt(0, 0, 0-10,  0, 0, 0-5,  0, 1, 0);
        else
            gluLookAt(camp.x, camp.y, camp.z, rc.x, rc.y, rc.z,  up.x, up.y, up.z);
        ...
        glPushMatrix();
            glSpace();
        glPopMatrix(); glPushMatrix();
            glTranslatef(sat_position.x, sat_position.y, sat_position.z);
            glRotatef(rot_sat, 0, 1, 0);
            glScalef(0.5, 0.5, 0.5);
            glSat();
        glPopMatrix(); glPushMatrix();
            glMaterialfv( GL_FRONT, GL_DIFFUSE, white);
...
void onKeyboard(unsigned char key, int x, int y) {
    bool ts = true;
    switch(key) {
        case 'q': rockets[0] = ts; break;
        case 'w': rockets[1] = ts; break;
        case 'd': rockets[2] = ts; break;
        case 'e': rockets[3] = ts; break;
        case 'a': rockets[4] = ts; break;
        case 's': rockets[5] = ts; break;
        /*
         * Az M1 elo kozvetitese a muhold befogasarol megtekintheto
         * a 't' billentyu lenyomasaval
         */
        case 't': camera_crew = ts;break;
    }
...
// az Idle esemenyek frekvenciajara csak a 0 a garantalt minimalis ertek
void onIdle( ) {
    long time = glutGet(GLUT_ELAPSED_TIME);
    rot_spaceship = time / -100.0f;
    rot_planet = time / 200.0f;
    rot_sat = time / 20.0f;

    orbit_spaceship = time / 50.0f;


    float ftime = (time - old_time)/ 1000.0f;
    float rpw = 200.0;

    float angl = rot_sat*M_PI/180.0f;
    Vector red_rocket(cos(angl),0,-1*sin(angl));
    Vector green_rocket(cos(angl+M_PI/2.0f),0,-1.0f*sin(angl+M_PI/2.0f));

    if(rockets[0]) {
        sat_speed = sat_speed - red_rocket * ftime * rpw;
    }
    if(rockets[1]) {
        sat_speed = sat_speed + red_rocket * ftime * rpw;
    }
    if(rockets[2]) {
        sat_speed.y += ftime * rpw;
    }
    if(rockets[3]) {
        sat_speed.y -= ftime * rpw;
    }
    if(rockets[4]) {
        sat_speed = sat_speed - green_rocket * ftime * rpw;
    }
    if (rockets[5]) {
        sat_speed = sat_speed + green_rocket * ftime * rpw;
    }

    sat_position = sat_position + sat_speed * ftime/1000.0;
    old_time = time;
    glutPostRedisplay();
